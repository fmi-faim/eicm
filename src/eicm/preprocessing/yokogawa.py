...
def parse_filename(file_name: str, plate_name: str):
    """
    Parse Yokogawa image file names.
    File name pattern:
    {plate_name}_{well}_{timepoint}{field}{L}{action}{Z-plane}{channel}.tif

    :param file_name:
    :param plate_name:
    :return: plate_name, well, time_point, field, l, action, z, channel
    """
    ...
    well, time_point, field, l, action, z, channel = regex.findall(tmp)[0]

    return plate_name, well, time_point, int(field), l, action, int(z), channel
...
def create_table(files: List[str], plate_name: str) -> pd.DataFrame:
    ...
    path = []
    for file in files:
        p, w, t, f, l_, a, z_, c = parse_filename(file, plate_name)
        plate.append(p)
        well.append(w)
        timepoint.append(t)
        field.append(f)
...
def get_output_name(acquistion_date: str, channel: Dict) -> str:
    """
    Create output name for reference images.

    :param acquistion_date:
    :param channel:
    :return: "{acquistion_date}_{ovjective}_{laser}_{filter}.tif"
    """
    obj = channel["objective"]
    laser = channel["laser"]
    filt = channel["filter"]
    return f"{acquistion_date}_{obj}_{laser}_{filt}.tif"
